...
load("TreeMod_LA_Lg_40_Stabil.RData")
...
for(t in TT2){   
#climate shift
	if(t <= 10) kq <- c(rep(11,(Plot.width*2)),kq[1:(CellNum-(Plot.width*2))])
	if(t > 10) kq <- c(rep(12,(Plot.width*2)),kq[1:(CellNum-(Plot.width*2))])
...
##where are the genotypes (adults)?  
 ...
 clim.state <- matrix(kq,ncol=Plot.width,nrow=Plot.length,byrow=T)
   ...
   jpeg(filename="ShiftPop_Tree_LA_Lg_alleles_40.jpg",width=9,height=7, units='in', res=500)
#jpeg(filename="ShiftPop_Tree_LA_Lg_alleles_80.jpg",width=9,height=7, units='in', res=500)
...
save.image(file="TreeMod_LA_Lg_40_Shift.RData")   
#save.image(file="TreeMod_LA_Lg_80_Shift.RData") 	 
