   ...
   Comp.A[,j] <- apply(PA[,j:length(JJ)],1,sum)
	...
	for(k in KK){
		for(i in II){               #germination and survival for each genotype (add competition)
			...
			germ <- max.germ*(TGclim[step[k,i]+1]-(0.003*sum(PA[k,])))
			...
			for(j in JJ){
			surv <- 0	
				if (i ==1) surv <- max.surv[j]*(SFclim.1.6[step[k,i]+1]-(CA.eff[j]*Comp.A[k,j]))
				if (i ==6) surv <- max.surv[j]*(SFclim.1.6[step[k,i]+1]-(CA.eff[j]*Comp.A[k,j]))
				if (i>1 & i<6) surv <- max.surv[j]*(SFclim.2.5[step[k,i]+1]-(CA.eff[j]*Comp.A[k,j]))
			if(surv<0) surv <- 0 
   ...
   sd.cell <- matrix(0,length(KK),length(II))     
	...
	for(k in KK){
	  if(sum(N[k,2:4,])>0){	
		...
		fec <- matrix(0,6,4); pol <- matrix(0,6,4)
		...
		for(i in II){   
			...
			Ov.num[k,1] <- (sum(N[k,2:4,1]*fec[1,2:4])+0.5*sum(N[k,2:4,2]*fec[2,2:4])+0.5*sum(N[k,2:4,4]*fec[4,2:4]))  #A1
			Ov.num[k,2] <- (sum(N[k,2:4,3]*fec[3,2:4])+0.5*sum(N[k,2:4,2]*fec[2,2:4])+0.5*sum(N[k,2:4,5]*fec[5,2:4]))  #A2
			Ov.num[k,3] <- (sum(N[k,2:4,6]*fec[6,2:4])+0.5*sum(N[k,2:4,4]*fec[4,2:4])+0.5*sum(N[k,2:4,5]*fec[5,2:4]))  #A3
			
			Pol.num[k,1] <- (sum(N[k,2:4,1]*pol[1,2:4])+0.5*sum(N[k,2:4,2]*pol[2,2:4])+0.5*sum(N[k,2:4,4]*pol[4,2:4]))  #A1
			Pol.num[k,2] <- (sum(N[k,2:4,3]*pol[3,2:4])+0.5*sum(N[k,2:4,2]*pol[2,2:4])+0.5*sum(N[k,2:4,5]*pol[5,2:4]))  #A2
			Pol.num[k,3] <- (sum(N[k,2:4,6]*pol[6,2:4])+0.5*sum(N[k,2:4,4]*pol[4,2:4])+0.5*sum(N[k,2:4,5]*pol[5,2:4]))  #A3
  ...
  sd.cell <- round(sd.cell) #round numbers of seeds to whole numbers
   ...
   for(k in KK){
		for(i in II){               #transition to next size class for each genotype 
			...
			trans <- 0
			  trans <- max.trans[j]*(TGclim[step[k,i]+1]-(CA.eff[j]*Comp.A[k,j]))
			  if(trans<0) trans <- 0
...
write.table(Pop.i,"StablePop_Peren_LA_20.txt")  
#write.table(Pop.i,"StablePop_Tree_LA_80.txt")    
   ...
   for(k in KK){
   	if(sum(N[k,2:4,II])>0){
   		 occ[k] <- 1
   		 prop1[k] <-  (sum(0.5*N[k,2:4,2])+sum(0.5*N[k,2:4,4])+sum(N[k,2:4,1]))/sum(N[k,2:4,II]) 
   		 prop2[k] <-  (sum(0.5*N[k,2:4,5])+sum(0.5*N[k,2:4,2])+sum(N[k,2:4,3]))/sum(N[k,2:4,II]) 
   		 prop3[k] <-  (sum(0.5*N[k,2:4,4])+sum(0.5*N[k,2:4,5])+sum(N[k,2:4,6]))/sum(N[k,2:4,II]) 
   ...
   jpeg(filename="StablePop_Peren_LA_alleles_20.jpg",width=9,height=7, units='in', res=500)
#jpeg(filename="StablePop_Tree_LA_alleles_80.jpg",width=9,height=7, units='in', res=500)
...
save.image(file="PerenMod_LA_20_Stabil.RData")   
#save.image(file="PerenMod_LA_80_Stabil.RData") 
