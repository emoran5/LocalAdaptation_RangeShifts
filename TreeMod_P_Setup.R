...
#Output: Map of initial distribution, text file of population numbers, dispersal matrices saved in R file 
...
########Tree population
 #Size classes - 5 size classes
   ...
   for(k in KK){
   #Climate 1...10
   	if(kq[k]==10) N.i[k,2,] <- 40  #saplings 
   	if(kq[k]==9)  N.i[k,2,] <- 80  #saplings 
   	if(kq[k]==8)  N.i[k,2,] <- 200 #saplings 
   	if(kq[k]==7)  N.i[k,2,] <- 200 #saplings 
   	if(kq[k]==6)  N.i[k,2,] <- 200 #saplings 
	if(kq[k]==5)  N.i[k,2,] <- 200  #saplings 
   	if(kq[k]==4)  N.i[k,2,] <- 200  #saplings
	if(kq[k]==3)  N.i[k,2,] <- 80 #saplings 
	if(kq[k]==2)  N.i[k,2,] <- 40  #saplings 
   }
   ...
   pop.state <- rep(0,length(KK))
      ...
      for(k in KK){
   	if(sum(N.i[k,2,II])>0)  pop.state[k] <-  1   		  	
   }
   	   		 
   mat.pop.state <- matrix(pop.state,ncol=Plot.width,nrow=Plot.length,byrow=T) 
     
 jpeg(filename="Initial_Tree_P.jpg",width=9,height=7, units='in', res=500)
...
#export as text file
Pop.i <- N.i
write.table(Pop.i,"InitialPop_Tree_P.txt") 
...
save.image(file="TreeMod_P_Setup.RData")