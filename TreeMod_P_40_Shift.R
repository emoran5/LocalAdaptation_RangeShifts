...
#Output:Map of distribution, text file of population numbers, saved workspace

load("TreeMod_P_40_Stabil.RData")
...
#export as text file
Pop.i <- N
write.table(Pop.i,"ShiftPop_Tree_P_40.txt")  
#write.table(Pop.i,"ShiftPop_Tree_P_80.txt")  
...
mat.pop.state <- matrix(pop.state,ncol=Plot.width,nrow=Plot.length,byrow=T)
 clim.state <- matrix(kq,ncol=Plot.width,nrow=Plot.length,byrow=T)
   ...
   jpeg(filename="ShiftPop_Tree_P_40.jpg",width=9,height=7, units='in', res=500)
#jpeg(filename="ShiftPop_Tree_P_80.jpg",width=9,height=7, units='in', res=500)
...
save.image(file="TreeMod_P_40_Shift.RData")   
#save.image(file="TreeMod_P_80_Shift.RData") 	 
