...
########Tree population
 ...
 jpeg(filename="Initial_Tree_LA_alleles.jpg",width=9,height=7, units='in', res=500)
...
write.table(Pop.i,"InitialPop_Tree_LA.txt") 
...
jpeg(filename="DispersalKernels_trees",width=6,height=6, units='in', res=500)
plot(x1,pd.40,xlim=c(0,300),type='l',lwd=2,ylab="probability density",xlab="distance (m)")
points(x1,pd.80,type='l',lwd=2,lty=2)
points(x1,pd.p,type='l',lwd=2,col='blue')
legend(200,4e-04,legend=c("40 m seed","80 m seed","pollen"),lwd=c(2,2,2),lty=c(1,2,1),col=c('black',"black","blue"),cex=0.8)
dev.off()
...
jpeg(filename="SeedDisp_tree_40.jpg",width=9,height=7, units='in', res=500)
#jpeg(filename="SeedDisp_tree_80.jpg",width=9,height=7, units='in', res=500)
Seeds <- SD.mat.40[3025,]*5000
#Seeds <- SD.mat.80[3025,]*5000

Seed.Mat <- matrix(Seeds,ncol=Plot.width,nrow=Plot.length,byrow=T) 

image(x=c((1:Plot.length)*20),y=c((1:Plot.width)*20),z=Seed.Mat,col=c("white","yellow","gold1","gold2","orange","dark orange","red"),breaks=c(0,0.00001,0.001,0.1,10,100,1000,3000),xlab="Distance",ylab="Distance",main="Seeds (out of 5000)") 
 legend(1500,950,legend=c("<0.00001","<0.001","<0.1","<10","<100","<1000","<3000"), fill=c("white","yellow","gold1","gold2","orange","dark orange","red") ,cex=0.7) 
dev.off() 

save.image(file="TreeMod_LA_Setup.RData")